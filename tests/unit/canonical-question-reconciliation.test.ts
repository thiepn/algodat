...
describe('Identitäts- und Sammlungsreview', () => {
  it('entscheidet neun Kandidaturen und führt den Rest explizit offen', () => {
    ...
    expect(review.identityDecisions.decisions).toHaveLength(9);
    expect(review.identityDecisions.openCandidateCount).toBe(75);
    expect(
  ...
  it('unterstützt Dubletten-, Split- und Solution-only-Entscheidungen ohne neue Fragen zu erfinden', () => {
    ...
    expect(QuestionIdentityDecisionsFileSchema.parse(fixture).decisions).toHaveLength(11);
  });
  ...
  it('segregiert blockierte Kandidaturen vom öffentlichen Korpus', () => {
    ...
    expect(corpus.questions).toHaveLength(9);
  });
